...
This directory contains the artifacts used to group the scaled-out attacker instances in a Function-as-a-Service (FaaS) environment. It focuses on:

- Spinning up identical attacker instances behind a shared URL.
- Exposing a simple endpoint to read out the underlying platform’s instance identifier.
- Using mem-lock and mem-check operations plus instance IDs to infer server-sharing relationships among scaled-out instances.
...
- Mem-lock and em-check primitives.
...
Python script that uses the lock/check/instance_id endpoints to infer server-sharing relationships among the scaled-out attacker instances that sit behind a shared service URL.
...
- In each iteration, the script:
  ...
  3. Compares each check result against a predefined threshold to determine which instances experienced elevated memory-access latency.
